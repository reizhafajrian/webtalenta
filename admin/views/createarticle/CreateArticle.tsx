...
const Modal = ({
  ...
  const handleSubmit = async () => {
    try {
      const postData = await fetch(server + "/api/admin/article", {
        headers: {
...
interface TableTrueProps {
  state: any;
  type: Boolean;
  deleteFunction: (id: any, article: any) => void;
  editFunction: (id: any, article: any) => void;
}
const TableTrue = ({
  state,
  type,
  ...
  const filterArray = state.data.article.filter(
  ...
  return (
    <CTable color="dark" striped>
      ...
      <CTableBody>
        {filterArray.map((article: any, index: number) =>
          type === true ? (
            <>
              <CTableRow>
                ...
                <CTableDataCell className={"text-center"}>
                  <CButton
                    color="primary"
                    onClick={(e) => editFunction(e, article)}
                  >
                    Edit
                  </CButton>
                  <CButton
...
const CreateArticle = () => {
  ...
  const deleteArticle = async (e: any, article: any) => {
    try {
      const postData = await fetch(`/api/admin/article`, {
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <>
      <CButton color="primary" className={"mb-2"} onClick={(e) => create(e)}>
        Create New
      </CButton>
